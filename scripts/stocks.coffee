...
module.exports = (robot) ->
    ...
    robot.respond /stock (.*)/i, (msg) ->    
        ...
        robot.http(url).get() (err, res, body) ->
            ...
            chart = "https://image-charts.com/chart?chtt=#{ticker}&chbh=a&chd=a:#{prices.join()}&chl=#{prices.join('|')}&chxl=0:|#{dates.join('|')}|&chxt=x&chs=999x250&cht=bvg".replace(/\|/g,'%7C')            
            msg.send "<#{chart}|#{ticker}>"
        msg.finish()            
